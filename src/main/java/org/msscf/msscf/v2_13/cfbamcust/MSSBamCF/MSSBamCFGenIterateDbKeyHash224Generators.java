...
public class MSSBamCFGenIterateDbKeyHash224Generators
	extends MssCFGenIteratorObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenIterateDbKeyHash224Generators() {
		super();
	}
	...
	public ListIterator<ICFLibAnyObj> enumerateDetails( MssCFGenContext genContext)
	{
		...
		ICFBamDbKeyHash224GenObj genObj;
		Iterator<ICFBamValueObj> almostDone = schemaTypes.iterator();
		List<ICFLibAnyObj> list = new LinkedList<ICFLibAnyObj>();
		while (almostDone.hasNext()) {
			valueObj = almostDone.next();
			if( valueObj instanceof ICFBamDbKeyHash224GenObj ) {
				genObj = (ICFBamDbKeyHash224GenObj)valueObj;
				list.add( genObj );
			}
		}
