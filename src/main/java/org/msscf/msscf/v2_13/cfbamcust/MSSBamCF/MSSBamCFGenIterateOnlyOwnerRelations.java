...
public class MSSBamCFGenIterateOnlyOwnerRelations
	extends MssCFGenIteratorObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenIterateOnlyOwnerRelations() {
		super();
	}
	...
	public ListIterator<ICFLibAnyObj> enumerateDetails( MssCFGenContext genContext)
	{
		...
		ListIterator<ICFBamRelationObj> ownerRelations = MSSBamCFTableObj.getOnlyOwnerRelations( tableObj ).listIterator();
		while (ownerRelations.hasNext()) {
			relation = ownerRelations.next();
			list.add( relation );
		}
