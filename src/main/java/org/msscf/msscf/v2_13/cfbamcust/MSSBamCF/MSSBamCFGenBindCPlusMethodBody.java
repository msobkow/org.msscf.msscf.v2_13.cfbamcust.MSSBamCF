...
public class MSSBamCFGenBindCPlusMethodBody
	extends MssCFGenBindObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenBindCPlusMethodBody() {
		super();
	}
	...
	public String expandBody( MssCFGenContext genContext ) {
		...
		ICFBamServerMethodObj serverMethod = (ICFBamServerMethodObj)genDef;

		String ret;
		String raw = serverMethod.getRequiredCppMethodBody();
		if( ( raw == null ) || ( raw.length() <= 0 ) ) {
			ret = "";
		}
		else {
			StringBuffer execName = new StringBuffer( serverMethod.getGenDefName() );
			execName.append( "::" );
			execName.append( serverMethod.getObjFullName() );
			execName.append( "::CPlusMethodBody" );
