...
public class MSSBamCFGenIterateSchemaSchemaOrderedRoles
	extends MssCFGenIteratorObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenIterateSchemaSchemaOrderedRoles() {
		super();
	}
	...
	public ListIterator<ICFLibAnyObj> enumerateDetails( MssCFGenContext genContext)
	{
		...
		List<ICFBamSchemaRoleObj> onlyMyChildrenSchemaRoles = new ArrayList<>();
		for (ICFBamSchemaRoleObj role: optionalChildrenSchemaRoles) {
			if (role.getOptionalDefSchemaId() == null && role.getOptionalDefSchemaTenantId() == null) {
				onlyMyChildrenSchemaRoles.add(role);
			}
		}
		int len = onlyMyChildrenSchemaRoles.size();
		ICFBamSchemaRoleObj[] arr = new ICFBamSchemaRoleObj[len];
		Iterator<ICFBamSchemaRoleObj> iter = onlyMyChildrenSchemaRoles.iterator();
		int idx = 0;
