...
public class MSSBamCFGenIterateDbKeyHash256Generators
	extends MssCFGenIteratorObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenIterateDbKeyHash256Generators() {
		super();
	}
	...
	public ListIterator<ICFLibAnyObj> enumerateDetails( MssCFGenContext genContext)
	{
		...
		ICFBamDbKeyHash256GenObj genObj;
		Iterator<ICFBamValueObj> almostDone = schemaTypes.iterator();
		List<ICFLibAnyObj> list = new LinkedList<ICFLibAnyObj>();
		while (almostDone.hasNext()) {
			valueObj = almostDone.next();
			if( valueObj instanceof ICFBamDbKeyHash256GenObj ) {
				genObj = (ICFBamDbKeyHash256GenObj)valueObj;
				list.add( genObj );
			}
		}
