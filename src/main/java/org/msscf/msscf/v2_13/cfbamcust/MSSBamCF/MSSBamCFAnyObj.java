...
//					}
	...
	public static boolean isTriviallyPrivate(ICFLibAnyObj what) {
		if (what == null) {
			...
			return( !isTriviallyProtected(what) && !isTriviallyPublic(what));
		}
	}
	...
	public static boolean isIntermediatelyPrivate(ICFLibAnyObj what) {
		...
		if(!isTriviallyPrivate(what)) {
			return(false);
		}
		else {
			return( !isIntermediatelyProtected(what) && !isIntermediatelyPublic(what));
		}
	}
	...
	public static boolean isPrivate(ICFLibAnyObj what) {
		...
		if(!isIntermediatelyPrivate(what)) {
			return(false);
		}
		else {
			return( !isProtected(what) && !isPublic(what));
		}
	}
	...
	public static boolean isSpecificallyPrivate(ICFLibAnyObj what) {
		...
		if (what instanceof ICFBamTableObj tbl) {
			...
			return(isSpecificallyPrivate(col.getRequiredContainerIndex().getRequiredContainerTable()) || isSpecificallyPrivate(col.getRequiredContainerIndex()) || isSpecificallyPrivate(col.getRequiredLookupColumn()));
		}
		else if (what instanceof ICFBamRelationColObj col) {
			return(isSpecificallyPrivate(col.getRequiredContainerRelation()) || isSpecificallyPrivate(col.getRequiredLookupFromCol().getRequiredLookupColumn()) || isSpecificallyPrivate(col.getRequiredLookupToCol().getRequiredLookupColumn()));
		}
		else if (what instanceof ICFBamIndexObj idx) {
			return(isSpecificallyPrivate(idx.getRequiredContainerTable()) || (idx.getRequiredCodeVis() == ICFBamSchema.CodeVisibilityEnum.Private));
		}
		else if(what instanceof ICFBamRelationObj reln) {
			return(isSpecificallyPrivate(reln.getRequiredContainerFromTable()) || isSpecificallyPrivate(reln.getRequiredLookupToTable()) || isSpecificallyPrivate(reln.getRequiredLookupFromIndex()) || isSpecificallyPrivate(reln.getRequiredLookupToIndex()) || (reln.getRequiredCodeVis() == ICFBamSchema.CodeVisibilityEnum.Private));
		}
		else if(what instanceof ICFBamParamObj param) {
			return(isSpecificallyPrivate(param.getRequiredContainerServerMeth().getRequiredContainerForTable()) || isSpecificallyPrivate(param.getRequiredContainerServerMeth()));
		}
		else if(what instanceof ICFBamServerMethodObj meth) {
			if (meth.getRequiredCodeVis() == ICFBamSchema.CodeVisibilityEnum.Private) {
				return(true);
			}
			else if (isSpecificallyPrivate(meth.getRequiredContainerForTable())) {
				return (true);
			}
			if (meth instanceof ICFBamServerObjFuncObj of) {
				if (isSpecificallyPrivate(of.getOptionalLookupRetTable())) {
					return(true);
				}
			}
			else if (meth instanceof ICFBamServerListFuncObj of) {
			...
			for (ICFBamParamObj p: meth.getOptionalComponentsParams()) {
				if (isSpecificallyPrivate(p)) {
					return( true);
				}
			}
			if (meth instanceof ICFBamServerProcObj) {
				return(true);
			}
			else {
				return(false);
