...
public class MSSBamCFGenBindIsSubservientRelation
	extends MssCFGenBindObj
{
	@SuppressWarnings("unused")
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenBindIsSubservientRelation() {
		super();
	}
	...
	public String expandBody( MssCFGenContext genContext ) {
		...
		if( genDef instanceof ICFBamRelationObj ) {
			...
			if (isSubservientRelation(relnToCheck)) {
				return( "yes" );
			}
			else {
				return( "no" );
	...
	public static boolean isSubservientRelation( ICFBamRelationObj relnToCheck ) {
		if (relnToCheck == null) {
			return false;
		}
