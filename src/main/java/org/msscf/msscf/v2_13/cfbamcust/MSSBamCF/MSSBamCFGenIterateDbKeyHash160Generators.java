...
public class MSSBamCFGenIterateDbKeyHash160Generators
	extends MssCFGenIteratorObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenIterateDbKeyHash160Generators() {
		super();
	}
	...
	public ListIterator<ICFLibAnyObj> enumerateDetails( MssCFGenContext genContext)
	{
		...
		ICFBamDbKeyHash160GenObj genObj;
		Iterator<ICFBamValueObj> almostDone = schemaTypes.iterator();
		List<ICFLibAnyObj> list = new LinkedList<ICFLibAnyObj>();
		while (almostDone.hasNext()) {
			valueObj = almostDone.next();
			if( valueObj instanceof ICFBamDbKeyHash160GenObj ) {
				genObj = (ICFBamDbKeyHash160GenObj)valueObj;
				list.add( genObj );
			}
		}
