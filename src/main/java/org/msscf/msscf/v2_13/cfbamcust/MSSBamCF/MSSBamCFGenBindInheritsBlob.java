...
public class MSSBamCFGenBindInheritsBlob
	extends MssCFGenBindObj
{
	private static final long serialVersionUID = 1L;

	public MSSBamCFGenBindInheritsBlob() {
		super();
	}
		...
		if (genDef instanceof ICFBamTableObj) {
			...
			while( tableDef != null ) {
				...
				while( tableColumns.hasNext() ) {
					...
					if( columnDef instanceof ICFBamBlobColObj ) {
						return( "yes" );
					}
					else if( columnDef instanceof ICFBamTableColObj ) {
						tableCol = (ICFBamTableColObj)columnDef;
						if( tableCol.getRequiredParentDataType() instanceof ICFBamBlobTypeObj ) {
							return( "yes" );
						}
					}
				}
